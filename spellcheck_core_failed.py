...
# Todos los locales de español detectados en tu instalación
SPANISH_LOCALE_CODES = [
    ("es", "AR"),
    ("es", "BO"),
    ("es", "CL"),
    ("es", "CO"),
...
# Solo los 2 ingleses más comunes
ENGLISH_LOCALE_CODES = [
    ("en", "US"),
    ("en", "GB"),
]
...
def build_locales(locale_codes):
    return [make_locale(lang, country) for lang, country in locale_codes]
...
def is_valid_in_any_locale(spell, word, locales):
    for locale in locales:
        try:
            if spell.isValid(word, locale, ()):
                return True
        except Exception:
            pass
    return False
...
def get_suggestions_any(spell, word, locales):
    suggestions = []
    seen = set()
...
def classify_word(word, suggestions_es, suggestions_en):
    all_suggestions = suggestions_es + suggestions_en
...
def check_word(word, spell, spanish_locales, english_locales):
    if should_ignore(word):
        return None

    valid_es = is_valid_in_any_locale(spell, word, spanish_locales)
    valid_en = is_valid_in_any_locale(spell, word, english_locales)

    if valid_es or valid_en:
        return None

    suggestions_es = get_suggestions_any(spell, word, spanish_locales)
    suggestions_en = get_suggestions_any(spell, word, english_locales)

    error_type = classify_word(word, suggestions_es, suggestions_en)
    if error_type not in ("tilde", "ortografia"):
        return None

    suggestions = []
    seen = set()
    for s in suggestions_es + suggestions_en:
        key = s.lower()
        if key not in seen:
            seen.add(key)
            suggestions.append(s)
...
def analyze_file(path):
    ...
    spanish_locales = build_locales(SPANISH_LOCALE_CODES)
    english_locales = build_locales(ENGLISH_LOCALE_CODES)
    ...
    try:
        ...
        for word in WORD_RE.findall(text):
            ...
            result = check_word(word, spell, spanish_locales, english_locales)
