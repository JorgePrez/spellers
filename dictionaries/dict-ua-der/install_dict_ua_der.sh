#!/bin/bash
# Instala dict-ua-der. Uso: sudo bash install_dict_ua_der.sh
set -eu
...
echo "==> Empaquetar .oxt"
rm -f "$OXT"
( cd "$SRC_DIR" && zip -q -r "$OXT" description.xml dictionaries.xcu ua_der_GT.aff ua_der_GT.dic META-INF/manifest.xml )
