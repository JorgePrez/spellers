...
def from_reference_dic() -> set[str]:
    """Harvest legal terms from es_GT/es_ES using LEGAL_MARKERS."""
    kept: set[str] = set()
    for name in ("es_GT.dic", "es_ES.dic"):
        ...
        for line in path.read_text(encoding="utf-8", errors="replace").splitlines()[1:]:
            ...
            if not valid(w):
                continue
            low = w.casefold()
            if any(m in low for m in LEGAL_MARKERS):
                kept.add(w)
    return kept


def load_expanded() -> set[str]:
    """Load expanded_der.txt if exists."""
    expanded_file = EXT / "expanded_der.txt"
