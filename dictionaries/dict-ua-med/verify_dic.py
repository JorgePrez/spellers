#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Valida ua_med_GT.dic (conteo, UTF-8, duplicados)."""
import re
import sys
from pathlib import Path

DIC = Path(__file__).resolve().parent / "ua_med_GT.dic"
...
def main() -> int:
    ...
    accented = sum(
        1 for w in words if re.search(r"[\u00e1\u00e9\u00ed\u00f3\u00fa\u00fc\u00f1]", w)
    )
    dupes = len(words) - len({w.lower() for w in words})
    ...
    if declared != len(words):
        ...
        return 1
    if dupes:
        print("ERROR: hay palabras duplicadas")
        return 1
