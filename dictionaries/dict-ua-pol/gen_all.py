# -*- coding: utf-8 -*-
"""Generate UA political science Hunspell dictionary (UTF-8). Orthography-first."""
    ...
    for name in ("es_GT.dic", "es_ES.dic"):
        ...
        for line in path.read_text(encoding="utf-8", errors="replace").splitlines()[1:]:
            ...
            if not valid(w):
                continue
            low = w.casefold()
            if any(m in low for m in POL_MARKERS):
                kept.add(w)
    return kept


def load_expanded() -> set[str]:
    """Load expanded_pol.txt if exists."""
    expanded_file = EXT / "expanded_pol.txt"
...
def collect() -> list[str]:
    ...
    dump = SRC / "ua_pol_lexicon_full.txt"
    dump.write_text(
        "# Lexico ciencia politica UA (generado)\n" + "\n".join(words) + "\n",
        encoding="utf-8",
        newline="\n",
    )
    return words
