#!/bin/bash
# Instala dict-ua-eco. Uso: sudo bash install_dict_ua_eco.sh
set -eu

SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
LO_PROG="/opt/libreoffice25.8/program"
OXT="/tmp/dict-ua-eco.oxt"
AFF_SRC="/opt/libreoffice25.8/share/extensions/dict-es/es_GT.aff"

echo "==> Preparar archivos"
sed -i 's/\r$//' "$SRC_DIR/ua_eco_GT.dic" "$SRC_DIR/ua_eco_GT.aff" "$SRC_DIR/description.xml" 2>/dev/null || true
...
for name in ("description.xml", "dictionaries.xcu", "ua_eco_GT.dic"):
    p = src / name
    if not p.exists():
        continue
    raw = p.read_bytes()
...
PY
if [ -f "$AFF_SRC" ]; then
  cp "$AFF_SRC" "$SRC_DIR/ua_eco_GT.aff"
  sed -i 's/\r$//' "$SRC_DIR/ua_eco_GT.aff"
fi
WORDS=$(tail -n +2 "$SRC_DIR/ua_eco_GT.dic" | sed '/^\s*$/d' | wc -l | tr -d ' ')
sed -i "1s/.*/$WORDS/" "$SRC_DIR/ua_eco_GT.dic"
echo "    Palabras: $WORDS"

echo "==> Empaquetar .oxt"
rm -f "$OXT"
( cd "$SRC_DIR" && zip -q -r "$OXT" description.xml dictionaries.xcu ua_eco_GT.aff ua_eco_GT.dic META-INF/manifest.xml )
...
"$LO_PROG/unopkg" remove --shared org.ua.dictionaries.eco-gt 2>/dev/null || true
